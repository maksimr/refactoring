...
describe('visitImports', function() {
  ...
  it('should visit require context with parameters', function() {
    const fn = jasmine.createSpy();
    visitImports(parse('require.context("foo", true, /foo/);'), fn);
    expect(fn).toHaveBeenCalled();
  });
});
...
describe('updateImports', function() {
  ...
  it('should update require context', function() {
    assert.equal(updateImports('require.context("foo", true, /bar/);', '/a/b/c.js', 'foo', 'bar'), 'require.context("bar", true, /bar/);');
  });
});
